...
export const POST = withHandler('api/users', 'POST', async (req) => {
  const { email, password, fullName, role, seniorityDate, teamIds, phone } = await parseBody(req);
  const user = await usersAPI.createUser(email, password, fullName, role, seniorityDate, teamIds ?? [], phone ?? null);
...
export const PATCH = withHandler('api/users', 'PATCH', async (req) => {
  const { id, fullName, email, role, seniorityDate, isActive, teamIds, onCallAvailable, scheduleStyle, skillRoles, phone, workAddress, commuteMinutes, preferredSmartDay, desiredSmartDaysPerMonth, birthDate } = await parseBody(req);
  ...
  if (birthDate !== undefined) updates.birth_date = birthDate || null; // 'YYYY-MM-DD' o vuoto
  ...
  if (scheduleStyle === 'stable' || scheduleStyle === 'random') updates.schedule_style = scheduleStyle;
  if (Array.isArray(skillRoles)) updates.skill_roles = skillRoles;
